...
初回起動はポート `8080` を使用します。このポートが既に使用されている場合は、コマンドライン引数または環境変数で上書きできます。

```bash
# Linux/macOS：コマンドライン引数
./pst --port 18080
...
Windows では PowerShell から一時的にポートを指定できます。

```powershell
.\pst.exe --port 18080
```

ダブルクリック起動時の既定ポートを固定するには、展開したディレクトリの `start.bat` をテキストエディターで開き、起動コマンドを次のように変更します。

```bat
start cmd /k .\pst.exe --port 18080
```

優先順位は `--port` > `PST_PORT` > `config.db` > 既定値 `8080` です。コマンドラインまたは環境変数で上書きした場合、`sav_cli` などの内部サービスが実際の待受ポートと同じ値を参照できるよう、最終ポートと `port_source` が `config.db` に書き込まれます。上書きが有効な間、PST 設定には実際のポートと上書き元が表示され、ポート入力は無効になり、設定 API もポート変更を拒否します。起動のたびに上書き元を再判定し、上書きを削除すると、`config.db` には最後のポートが残り、`port_source` は空になり、Web 画面から再び変更できます。

Web 画面でポート、TLS、その他の起動設定を変更した場合は、保存後に PST を再起動してください。

> [!IMPORTANT]
> 起動ポート専用の `--port` と `PST_PORT` を除き、PST は `config.yaml`、`-config` 引数、その他の PST 設定用環境変数を読み込みません。アップグレード時は以前の値を Web 設定へ手動でコピーし、古いファイルと変数を削除してください。
...
コンテナ内の PST 待受ポートを上書きする場合は、ポートマッピングを変更し、同時に `PST_PORT` を渡します。例：
...
永続設定はすべて現在の作業ディレクトリにある `config.db` に保存されます。起動時に `--port` または `PST_PORT` で決定した最終ポートと上書き元もこのデータベースに書き込まれます。次の旧設定方法は削除され、互換目的でも読み込まれません。
